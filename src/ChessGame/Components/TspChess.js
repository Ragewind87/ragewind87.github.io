...
const TspChess = (props) => {
    ...
    const swapTurn = () => {
        if (currentPlayer === Players.WHITE) setCurrentPlayer(Players.BLACK);
        else setCurrentPlayer(Players.WHITE);
    };
    ...
    const squareClicked = (y, x) => {
        ...
        else if (boardData.isGameOver || promoteInProgress) return;
        ...
        // if you click on YOUR OWN piece...
            ...
            switch (boardMap[y][x].pcType) {
                case Pieces.ROOK:
                    showRookMoves(y, x, true);
                    break;
                case Pieces.KNIGHT:
                    showKnightMoves(y, x, true);
                    break;
                case Pieces.BISHOP:
                    showBishopMoves(y, x, true);
                    break;
                case Pieces.QUEEN:
                    showQueenMoves(y, x, true);
                    break;
                case Pieces.KING:
                    showKingMoves(y, x, true);
                    break;
                case Pieces.PAWN:
                    showPawnMoves(y, x, true);
                    break;
                default:
                    console.log('ERROR: DEFAULT CASE REACHED IN squareClicked() movement highlighting');
            }
        }
        ...
        // This is a successful move so the turn is swapped to the next player upon completion
            ...
            if (currentPlayer === Players.WHITE)
                // clears ability to en-passant
                clearBlackEP();

            if (currentPlayer === Players.BLACK)
                // from any pawns that had it from last turn
                clearWhiteEP();

            // if a piece was captured, add it to the dungeon
            if (boardMap[y][x].pcType !== Pieces.EMPTY) addToDungeon(y, x);
                ...
                if (currentPlayer === Players.BLACK) {
                    ...
                    if (y === selectedSquare[0] + 2) {
                        if (
                            ...
                            boardMap[y][x - 1].pcOwner === Players.WHITE
                        ) {
                            // grants adjacent enemy pawns
        ...
        function runEndOfTurnChecks() {
            ...
            if (boardData.whiteCheck && !whiteCanMove) boardData.whiteCheckMate = true;
            else if (boardData.blackCheck && !blackCanMove) boardData.blackCheckMate = true;

            // determines STALEMATE status
            if ((!boardData.whiteCheck && !whiteCanMove) || (!boardData.blackCheck && !blackCanMove)) {
        ...
        function addToDungeon(y, x) {
            ...
            if (!props.test) dungeon.appendChild(node);
        }
        ...
        function deHighlightAllSquares() {
            ...
            for (let i = 0; i < castleRooks.length; i++) boardMap[castleRooks[i].y][castleRooks[i].x].canCastle = false;
            setCastleRooks([]);
            setHighlights([]);
            setCapturables([]);
        }
        ...
        function playerIsInCheck(player) {
            return (
                (player === Players.WHITE && boardData.whiteCheck) || (player === Players.BLACK && boardData.blackCheck)
            );
        }

        // determines if the current player's king is safe
        function playerKingIsSafe(player) {
            if (player === Players.WHITE) return squareIsSafe(boardData.wKingY, boardData.wKingX);
            else return squareIsSafe(boardData.bKingY, boardData.bKingX);
        }
        ...
        function updatePieceListRightCastle() {
            if (currentPlayer === Players.WHITE) {
                for (let i = 0; i < boardData.whitePieces.length; i++) {
                    if (boardData.whitePieces[i].y === 7 && boardData.whitePieces[i].x === 7) {
                ...
                for (let i = 0; i < boardData.blackPieces.length; i++) {
                    if (boardData.blackPieces[i].y === 0 && boardData.blackPieces[i].x === 7) {
        ...
        function updatePieceListLeftCastle() {
            if (currentPlayer === Players.WHITE) {
                for (let i = 0; i < boardData.whitePieces.length; i++) {
                    if (boardData.whitePieces[i].y === 7 && boardData.whitePieces[i].x === 0) {
                        boardData.whitePieces[i].y = 7;
                ...
                for (let i = 0; i < boardData.blackPieces.length; i++) {
                    if (boardData.blackPieces[i].y === 0 && boardData.blackPieces[i].x === 0) {
                        boardData.blackPieces[i].y = 0;
        ...
        function updatePieceListsNormalMove() {
            ...
            if (currentPlayer === Players.WHITE) {
                for (let i = 0; i < boardData.whitePieces.length; i++) {
                    if (
                        boardData.whitePieces[i].y === selectedSquare[0] &&
                ...
                for (let i = 0; i < boardData.blackPieces.length; i++) {
                    if (
                        boardData.blackPieces[i].y === selectedSquare[0] &&
        ...
        function determineDraw() {
            ...
            // king vs king
            if (boardData.whitePieces.length === 1 && boardData.blackPieces.length === 1) {
                return true;
            }

            // lone white king, black bishop + king
            if (boardData.whitePieces.length === 1 && boardData.blackPieces.length === 2) {
                piece1 = boardData.blackPieces[0];
                piece2 = boardData.blackPieces[1];
                if (
                    boardMap[piece1.y][piece1.x].pcType === Pieces.BISHOP ||
            ...
            if (boardData.blackPieces.length === 1 && boardData.whitePieces.length === 2) {
            ...
            if (boardData.whitePieces.length === 1 && boardData.blackPieces.length === 2) {
                ...
                piece2 = boardData.blackPieces[1];
                if (
                    boardMap[piece1.y][piece1.x].pcType === Pieces.KNIGHT ||
            ...
            if (boardData.blackPieces.length === 2 && boardData.whitePieces.length === 2) {
                ...
                for (let i = 0; i < 2; i++) {
                    if (boardMap[boardData.whitePieces[i].y][boardData.whitePieces[i].x].pcType === Pieces.BISHOP) {
                        ...
                        for (let k = 0; k < 2; k++) {
                            if (
                                boardMap[boardData.blackPieces[k].y][boardData.blackPieces[k].x].pcType ===
                                Pieces.BISHOP
                            ) {
                                blackBishop = boardData.blackPieces[k];

                                // calculates if bishops are on same color square
                                if ((blackBishop.y + blackBishop.x) % 2 === (whiteBishop.y + whiteBishop.x) % 2) {
                                    return true;
                                }
                                break;
                            }
        ...
        function canWhiteMove() {
            ...
            for (let i = 0; i < boardData.whitePieces.length; i++) {
                ...
                switch (boardMap[piece.y][piece.x].pcType) {
                    case Pieces.ROOK:
                        canMove = showRookMoves(piece.y, piece.x, false) > 0;
                        break;
                    case Pieces.KNIGHT:
                        canMove = showKnightMoves(piece.y, piece.x, false) > 0;
                        break;
                    case Pieces.BISHOP:
                        canMove = showBishopMoves(piece.y, piece.x, false) > 0;
                        break;
                    case Pieces.QUEEN:
                        canMove = showQueenMoves(piece.y, piece.x, false) > 0;
                        break;
                    case Pieces.KING:
                        canMove = showKingMoves(piece.y, piece.x, false) > 0;
                        break;
                    case Pieces.PAWN:
                        canMove = showPawnMoves(piece.y, piece.x, false) > 0;
                        break;
                    default:
                        console.log('ERROR: DEFAULT CASE REACHED IN canWhiteMove()');
                }
                if (canMove) break;
            }
            return canMove;
            ...
            for (let i = 0; i < boardData.blackPieces.length; i++) {
                ...
                switch (boardMap[piece.y][piece.x].pcType) {
                    ...
                    case Pieces.PAWN:
                        ...
                        break;
                    default:
                        console.log('ERROR: DEFAULT CASE REACHED IN canBlackMove()');
                }
                if (canMove) break;
            }
            return canMove;
        ...
        function addDirectionalMoves(y, x, mainSquareOwner, goodMoves, direction) {
            ...
            switch (direction) {
                ...
                case Directions.UP:
                    yDir = -1;
                    yLimit = -1;
                    break; // sets up logic variables based on
                case Directions.LEFT:
                    xDir = -1;
                    xLimit = -1;
                    break; // direction being searched
            ...
            if (direction === Directions.UP || direction === Directions.DOWN) {
                ...
                for (let curY = y + yDir; curY !== y + 8 * yDir && curY !== yLimit; curY += yDir) {
                    ...
                    if (boardMap[curY][x].pcOwner === mainSquareOwner)
                        // discard if you run into your own piece
                        break;
                    else {
                        // otherwise...
                        ...
                        if (kingIsSafe) {
                            ...
                            // if you're NOT in check, this move will put us
                            if (!playerIsInCheck(mainSquareOwner))
                                // IN check, so we stop looking for new move
                                break;
                        } // however if you ARE in check, keep looking for a move
                        if (squareType !== Pieces.EMPTY)
                            // unless this was an attack. if it was, we have to stop
                            break;
            ...
            // LEFT and RIGHT
                ...
                for (let curX = x + xDir; curX !== x + 8 * xDir && curX !== xLimit; curX += xDir) {
                    ...
                    if (boardMap[y][curX].pcOwner === mainSquareOwner) break;
                    else {
                        ...
                        if (kingIsSafe) {
                            ...
                            if (!playerIsInCheck(mainSquareOwner)) break;
                        }
                        if (squareType !== Pieces.EMPTY) break;
                    }
                }
            }
            ...
            // DIAGONALS
                ...
                for (
                    let curY = y + yDir, curX = x + xDir;
                    curY !== y + 8 * yDir && curX !== x + 8 * xDir && curY !== yLimit && curX !== xLimit;
                    curY += yDir, curX += xDir
                ) {
                    squareType = boardMap[curY][curX].pcType;
                    squareOwner = boardMap[curY][curX].pcOwner;

                    if (boardMap[curY][curX].pcOwner === mainSquareOwner) break;
                    else {
        ...
        function squareIsSafe(y, x) {
            ...
            for (let checkY = y + 1; checkY < 8; checkY++) {
                ...
                if (squareBeingSearched.pcOwner === mainSquareOwner)
                    // search ran into your own piece.
                    break; // stop searching in this direction

                if (checkY === y + 1 && squareBeingSearched.pcType === Pieces.KING) {
                    // ran into a NEARBY enemy king
                    return false; // we're in danger
                }
                if (
                    squareBeingSearched.pcType === Pieces.QUEEN || // ran into an enemy queen or rook
                ...
                if (squareBeingSearched.pcType !== Pieces.EMPTY)
                    // we ran into something non-threatening
                    break; // stop searching in this direction
            }
            ...
            for (let checkY = y - 1; checkY > -1; checkY--) {
                ...
                if (squareBeingSearched.pcOwner === mainSquareOwner) break;

                if (checkY === y - 1 && squareBeingSearched.pcType === Pieces.KING) {
                    return false;
                }
                if (squareBeingSearched.pcType === Pieces.QUEEN || squareBeingSearched.pcType === Pieces.ROOK) {
                    return false;
                }
                if (squareBeingSearched.pcType !== Pieces.EMPTY) break;
            }
            ...
            for (let checkY = y - 1, checkX = x - 1; checkY > -1 && checkX > -1; checkY--, checkX--) {
                ...
                if (
                    checkY === y - 1 &&
                    checkX === x - 1 && // pawns only can attack in the direction
                    (squareBeingSearched.pcType === Pieces.KING || // they move, so need to check ownership
                        (squareBeingSearched.pcType === Pieces.PAWN && mainSquareOwner === Players.WHITE))
                ) {
                    // ran into a NEARBY enemy pawn or King.
                    return false; // we're in danger
                }
                if (squareBeingSearched.pcType === Pieces.BISHOP || squareBeingSearched.pcType === Pieces.QUEEN) {
                    // ran into an enemy Bishop or Queen.
                    return false; // we're in danger
                }
                if (squareBeingSearched.pcType !== Pieces.EMPTY) break; // we ran into something non-threatening
            ...
            for (let checkY = y + 1, checkX = x - 1; checkY < 8 && checkX > -1; checkY++, checkX--) {
                ...
                if (
                    ...
                    checkX === x - 1 &&
                    (squareBeingSearched.pcType === Pieces.KING ||
                        (squareBeingSearched.pcType === Pieces.PAWN && mainSquareOwner === Players.BLACK))
                ) {
                    return false;
                }
                if (squareBeingSearched.pcType === Pieces.BISHOP || squareBeingSearched.pcType === Pieces.QUEEN) {
            ...
            for (let checkY, checkX, i = 0; i < knightAttack.length; i++) {
                ...
                if (
                    boardMap[checkY][checkX].pcType === Pieces.KNIGHT &&
        ...
        function showKingMoves(y, x, highlight) {
            ...
            for (let yOffset = -1; yOffset < 2; yOffset++) {
                // adds 1 move in every direction
                for (let xOffset = -1; xOffset < 2; xOffset++) {
                    // skips the Square the king is already on
            ...
            // right castling
            if (
                boardMap[y][x].hasMoved === false && // adds right castle if pre-conditions
            ...
            // left castling
            if (
                boardMap[y][x].hasMoved === false && // adds left castle if pre-conditions
            ...
            for (let i = 0; i < possibleMoves.length; i++) {
                ...
                if (move.y > 7 || move.y < 0)
                    // immediately discard if move is off board
                    continue; // or collides with your own piece
                if (move.x > 7 || move.x < 0) continue;
                if (boardMap[move.y][move.x].pcOwner === kingOwner) continue;
                ...
                if (move.isCastleMove) {
                    ...
                    if (
                        (kingOwner === Players.WHITE && boardMap[7][8].whiteCheck) || // cant castle if you're in check
                        (kingOwner === Players.BLACK && boardMap[7][8].blackCheck)
                    )
                        continue;

                    if (move.castleSide === 'R') {
                        // sets variables based on castle direction
                        xDir = 1;
                        xLimit = 7;
                    ...
                    if (!kingIsSafe)
                        // if he is, discard this move
                        continue;

                    // castling move is good
                    // add move / rook highlighting
                    if (move.castleSide === 'R') {
                        if (kingOwner === Players.WHITE) castleRooks.push(new Move(7, 7));
                        else castleRooks.push(new Move(0, 7));
                    } else {
                        if (kingOwner === Players.WHITE) castleRooks.push(new Move(7, 0));
                        else castleRooks.push(new Move(0, 0));
                    }
                    safeMoves.push(move);
                }
                ...
                // all other non-castling moves
                    ...
                    if (squareIsSafe(move.y, move.x))
                        // checks if the king is still safe if he moves here
        ...
        function showPawnMoves(y, x, highlight) {
            ...
            // WHITE pawn moves
                ...
                for (let curY = y - 1; curY >= y - 2 && curY > -1; curY--) {
                    if (boardMap[curY][x].pcType !== Pieces.EMPTY) break;
                    possibleMoves.push(new Move(curY, x));
                }
            }
            ...
            for (let i = 0; i < possibleMoves.length; i++) {
                ...
                if (move.x > 7 || move.x < 0) continue; // discard if move is off the board
                if (move.y > 7 || move.y < 0) continue;
                if (move.y === y + 2 && y !== 1)
                    // discard double moves if not on starting row
                    continue;
                if (move.y === y - 2 && y !== 6) continue;
                if (boardMap[move.y][move.x].pcOwner === pawnOwner)
                    // discard if attacking your own piece
                    continue;

                // if you're white and CANNOT en-passant...
                // AND there is NOT a black piece to your diagonal, then discard this move
                if (
                    pawnOwner === Players.WHITE &&
                    boardMap[move.y][move.x].pcOwner !== Players.BLACK &&
                ...
                if (
                    pawnOwner === Players.BLACK &&
                    boardMap[move.y][move.x].pcOwner !== Players.WHITE &&
                    ((boardMap[y][x].canEpLeft === false && move.x === x + 1) ||
                        (boardMap[y][x].canEpRight === false && move.x === x - 1))
                ) {
                    continue;
                ...
                if (move.hasCapturable) {
                    // and if the move involves en-passant
                    if (pawnOwner === Players.WHITE) {
                        // make sure the captured piece
                ...
                if (kingIsSafe) {
                    ...
                    if (move.hasCapturable)
                        // add any en-passant capturables
        ...
        function showKnightMoves(y, x, highlight) {
            ...
            for (let i = 0; i < possibleMoves.length; i++) {
                ...
                if (move.y > 7 || move.y < 0)
                    ...
                    continue; // or collides with your own piece
                if (move.x > 7 || move.x < 0) continue;
                if (boardMap[move.y][move.x].pcOwner === knightOwner) continue;
                ...
                if (kingIsSafe) {
                    ...
                    // King is NOT SAFE if you make this move.
                    if (!playerIsInCheck(knightOwner))
                        // if you're NOT in check, this move will put us
                        break; // IN check, so stop looking for new moves
                    else continue; // however if you ARE in check, keep testing moves
            ...
            if (highlight) highlightGoodMoves(goodMoves);
    ...
    return (
        <div className="TspChess">
            <div style={{ display: 'flex', flexDirection: 'row', width: '100%' }}>
                <div style={{ display: 'flex', flexDirection: 'column', textAlign: 'center', width: '100%' }}>
                    ...
                    <div className="SubTitle" style={{ marginTop: '5px' }}>
                        {'Joe Kurtz  |  Nick McCarter  |  Justin Martin  |  Caleb Melnychenko'
                            .split('|')
                            .join('\u00A0\|\u00A0')}
                    </div>
            ...
            />
            <Board bState={boardState} pieceClicked={squareClicked} theme={getTheme} />
            <div
                style={{
                    display: 'flex',
                    flexDirection: 'row',
