...
import {
	GridSection, type IGridSectionProps, type IMainGrid, Status as CellStatus,
} from './GridSection.js';
...
export const ConnectFour: React.FunctionComponent = properties => {
	...
	const renderGrid = (): React.ReactNode => {
		...
		const grid
		= <Stack>
			{playGrid.grid.map((gridRow, i) => renderRow(gridRow, i))}
		</Stack>;
	...
	const getPlayerImage = (playerId: number): string => playersReference.current?.find(p => p.id === playerId)?.options?.icon ?? '';
	...
	const getLeftPanelBgImage = (): string | undefined => `url(${playersReference.current?.find(p => p.id === playerTurn)?.options?.background ?? ''})`;
...
&& <div style={{
display: 'flex', justifyContent: 'flex-end', alignSelf: 'top', width: '100%', height: '36%',
}}>
<Stack style={{
...
{/* Text */}
<div style={{
textAlign: 'center', width: '100%', paddingTop: '15px', border: '1px solid white',
}}>
<div style={{
fontFamily: 'Trocchi', fontSize: '24px', fontWeight: '700', color: 'white', lineHeight: '20px',
}}>
{playerTurn === 0 ? '' : `Player ${playerTurn}`}
</div >
<div style={{
fontSize: '17px', fontWeight: '300', color: '#ffcc66', marginBottom: '10px',
}}>
Turn
</div>
</div>
			...
			&& <Stack>
				...
				<div style={{
					marginTop: '-30px', marginLeft: 'auto', marginRight: 'auto',
				...
				>
					...
					<div id={'bottom'}
						...
						<button onClick={onResetButtonClick}
							style={{
								width: '80%', height: '30px', marginLeft: 'auto', marginRight: 'auto', overflow: 'clip',
							}}>
Reset Game
						</button>
					</div>
